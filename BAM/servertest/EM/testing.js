		...
		paramTypes.forEach(function(paramType) {
			...
			for (let index in results[paramType]) {
				...
				val = results[paramType][index]
								searchData[paramType].push(val);
				document.getElementById('selected_' + paramType).innerHTML += '<li id="i' + val + '">' + val + ' <button id="b' + val + '" class="remove-button" onclick="removeParam(\'' + paramType + '\', \'' + val + '\')"><u>remove</u></button></li>';
